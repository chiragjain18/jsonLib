 ...
 * By default JsonLib converts application classes to JSON using its built-in type
 * adapters. If JsonLib's default JSON conversion isn't appropriate for a type,
 ...
 *     }
 *   }}</pre>
 * With this type adapter installed, JsonLib will convert {@code Points} to JSON as
 ...
 * registering the type adapter with JsonLib. If your {@code JsonLib} instance
 ...
 *
 * <p>To use a custom type adapter with JsonLib, you must <i>register</i> it with a
...
// <p>A type adapter registered with JsonLib is automatically invoked while serializing
...
public abstract class TypeAdapter<T> {
   ...
   * Unlike JsonLib's similar {@link JsonLib#stringToJson(JsonElement, Appendable) stringToJson}
   ...
   * <pre>   {@code
   *
   * JsonLib gson = new GsonBuilder().registerTypeAdapter(Foo.class,
   *   new TypeAdapter<Foo>() {
  ...
  /**
   * Converts {@code value} to a JSON document. Unlike JsonLib's similar {@link
   * JsonLib#stringToJson(Object) stringToJson} method, this write is strict. Create a {@link
   * JsonWriter#setLenient(boolean) lenient} {@code JsonWriter} and call
   * {@link #write(JsonWriter, Object)} for lenient
   * writing.
   *
  ...
  /**
   * Converts the JSON document in {@code in} to a Java object. Unlike JsonLib's
   * similar {@link JsonLib#fromJson(java.io.Reader, Class) fromJson} method, this
  ...
  /**
   * Converts the JSON document in {@code json} to a Java object. Unlike JsonLib's
   * similar {@link JsonLib#fromJson(String, Class) fromJson} method, this read is
